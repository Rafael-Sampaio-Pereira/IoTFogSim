...
class PublisherApp(StandardApplicationComponent):
    ...
    def publish(self):
        ...
        self.simulation_core.updateEventsCounter(
            self.screen_name+" - sending MQTT PUBLISH REQUEST")
    ...
    def dataReceived(self, data):
        ...
        try:
            for package in self._buffer:
                destiny_addr, destiny_port, source_addr, source_port, _type, payload = self.extract_package_contents(
                    package)
...
class BrokerApp:
    ...
    def start(self, addr, port):
        ...
        broker_factory = BrokerFactory(
            self.visual_component, self.simulation_core)
        broker_factory.noisy = False
        # starting message broker server - Rafael Sampaio
        endpoints.serverFromString(reactor, "tcp:interface={}:{}".format(
            addr, port)).listen(broker_factory)
        # updating broker name (ip:port) on screen - Rafael Sampaio
        self.simulation_core.canvas.itemconfig(
            self.visual_component.draggable_name, text=str(addr+":"+str(port)))
...
class BrokerProtocol(StandardApplicationComponent):
    ...
    def send_mqtt_acknowledgement(self, destiny_addr, destiny_port):
        ...
        self.source_addr = self.transport.getHost().host
        self.source_port = self.transport.getHost().port
        response_package = self.build_package(
            "MQTT_ACK"+str(uuid.uuid4().fields[-1]), 'mqtt')
        self.send(response_package)
